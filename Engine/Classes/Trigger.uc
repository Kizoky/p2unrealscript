...
#exec Texture Import File=Textures\trigger.bmp Name=S_Trigger Mips=Off MASKED=1
...
function bool IsRelevant( actor Other )
{
	if( !bInitiallyActive )
		return false;
	if (bTriggerOnceOnly && !bCollideActors) // Fix for volume triggers and bTriggerOnceOnly
		return false;
	switch( TriggerType )
	{
	case TT_HumanPlayerProximity:
		...
		case TT_PlayerProximity:
			// RWS CHANGE: Merged "was" check from UT2003
