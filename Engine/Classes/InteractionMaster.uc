...
// Launch a DLC title. If successful, the current game is exited and the DLC title is launched. If it fails, FALSE is returned.
native function bool LaunchDLC(int AppID, optional bool bCreateShortcut);

// Launch a social network URL. Returns a null string on success, error message on failure.
native function string LaunchSocialNetwork(ESocialNetwork SocialNetwork);
...
// RWS BUGFIX: http://mail.epicgames.com/listarchive/showpost.php?list=unprog&id=28164&lessthan=&show=20
	if (ViewportOwner != none)
...
//event bool Process_KeyType( array<Interaction> InteractionArray, out EInputKey Key ) // Process a single key press
{
	...
	for ( Index=0; Index<InteractionArray.Length; Index++) 
	{
		...
		// RWS CHANGE: Merged from 2110
		if ( ( InteractionArray[Index].bActive ) && ( InteractionArray[Index].KeyType(key,Unicode) ) )
