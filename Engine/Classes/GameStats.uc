...
//
// Expanded for Postal 2 in Multibase.MpGameStats
// 
// ====================================================================
...
function Init()
{
	...
	if (TempLog!=None)
	{
		LogFileName = "Stats"$FullTimeDateNoSpace();
		log("Output Game stats to: "$LogFileName);
		TempLog.OpenLog(LogFileName);
	}
	else
	{
...
{
//	log(self$" logf called, log string "$logstring$" templog "$templog);
	if (TempLog!=None)
...
// Internals
///////////////////////////////////////////////////////////////////////////////
// Used to call Init here, but we want it to create a new file each time, so we now call it in
// NewGame
//event PostBeginPlay()
//{
//	Super.PostBeginPlay();
...
///////////////////////////////////////////////////////////////////////////////
// Date/Time with no spaces and in a format that windows allows for file names.
// The year is always 4 digits, all other numbers are two digits. 
// Year, Month, Day, Hour, Minute, Second
///////////////////////////////////////////////////////////////////////////////
function String FullTimeDateNoSpace()		
{
	return ""$Level.Year$TwoDigitString(Level.Month)$TwoDigitString(Level.Day)
			$TwoDigitString(Level.Hour)$TwoDigitString(Level.Minute)$TwoDigitString(Level.Second);
} 
...
function NewGame()
{
	...
	// Make a new stats file each time we start a new match
	Init();

	// Print out new game data
	tab				= ""$Chr(9);
...
function EndGame(string Reason)
{
	...
	// RWS change we don't have playernum
...
function ConnectEvent(PlayerReplicationInfo Who)
{
	...
	// RWS Change player num no in our controller
//	out = ""$Header()$"C"$Chr(9)$Controller(Who.Owner).PlayerNum$Chr(9);
	out = ""$Header()$"C"$Chr(9)$Who.PlayerName$Chr(9);
...
//	out = ""$Header()$"D"$Chr(9)$Controller(Who.Owner).PlayerNum;	//"Disconnect"
	out = ""$Header()$"D"$Chr(9)$Who.PlayerName;	//"Disconnect"
...
//	Logf( ""$Header()$"S"$chr(9)$Controller(Who.Owner).PlayerNum$chr(9)$Points$chr(9)$Desc );	//"Score"
	Logf( ""$Header()$"S"$chr(9)$Who.PlayerName$chr(9)$Points$chr(9)$Desc );	//"Score"
}
...
function KillEvent(string Killtype, PlayerReplicationInfo Killer, PlayerReplicationInfo Victim, class<DamageType> Damage)
{
	...
	if (Killer!=None)
	{
		// RWS Change we have no playernum
		//out = out$Controller(Killer.Owner).PlayerNum$Chr(9);
		out = out$Killer.PlayerName$Chr(9);
	...
	out = out$Victim.PlayerName$chr(9)$GetItemName(string(Controller(Victim.Owner).GetLastWeapon()));
