...
//
// 8-11 - backported AWGrenade stuff. - K
///////////////////////////////////////////////////////////////////////////////
class GrenadeProjectile extends P2Projectile;

var float MinChargeTime;		// What's the min time for how long it can be charged. This 
								// Determines the min velocity.
							...
							Vector momentum, class<DamageType> damageType)
{
	...
	if(GrenadeAltProjectile(self) != None
...
simulated function MakePickup(Pawn Other)
{
		...
		&& Other != None)
	{
		if(Role == ROLE_Authority)
		{
			...
			gp = spawn(class'GrenadePickup',,,Other.Location);
			if(gp != None)
			{
...
simulated function ProcessTouch(Actor Other, Vector HitLocation)
{
		...
		&& Pawn(Other).Health > 0)
	{
		...
		if(!bArmed
			&& GrenadeAltProjectile(self) != None
...
defaultproperties
{
	TransientSoundRadius=60
	 ...
	 UpRatio=0.45
	 ExploWallOut=0
	 GrenadeBounce=Sound'WeaponSounds.grenade_bounce'
	 bProjTarget=true
