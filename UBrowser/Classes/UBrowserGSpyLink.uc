...
var int 					ServerRegion;			// Region of the game server
...
function HandleServer(string Text)
{
	...
	OwnerFactory.FoundServer(Address, int(Port), "", GameSpyGameName());
}
...
function GotMatch(int MatchData)
{
	...
	case FoundSecret:
		Enable('Tick');
		SendBufferedData("\\gamename\\"$GameSpyGameName()$"\\location\\"$ServerRegion$"\\validate\\"$GameSpyValidate(WaitResult)$"\\final\\");
...
Begin:
	Enable('Tick');
	SendBufferedData("\\list\\\\gamename\\"$GameSpyGameName()$"\\final\\");
