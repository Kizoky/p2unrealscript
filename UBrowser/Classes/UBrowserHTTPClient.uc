...
event Opened()
{
	...
	SendBufferedData("User-Agent: Postal2"$CR$LF);
	...
	do
	{
		...
		case WaitingForHeader:
			...
			if(bGotData)
			{
				...
				// RWS CHANGE: Handle 302 - Resolving DNS Change. ie: postal2.com -> www.gopostal.com
		...
		case ReceivingHeader:
			...
			if(bGotData)
			{
				// RWS CHANGE: Handle 302 - Get the new 'real' location
				if(InStr(Line, "Location:") != -1)
				{
					Line = Right(Line, Len(Line) - 10);
					Line = Right(Line, Len(Line) - 7);
					ServerAddress = Left(Line, InStr(Line, "/"));
					NextState = WaitingForHeader;
					ResetBuffer();
					DoBind();
					return;
				}
