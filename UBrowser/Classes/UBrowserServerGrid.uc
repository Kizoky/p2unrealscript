...
{
	Server	= AddColumn(ServerName, 290);
	GameType= AddColumn(GameTypeName, 0);
	Ping	= AddColumn(PingName, 50);
	MapName	= AddColumn(MapNameName, 150);
...
function Resized()
{
	...
	// RWS Change: Size column for game type only if showing
	if(bShowGameTypes)
	{	// showing game type column
		Server.WinWidth		= W*0.27;
		GameType.WinWidth	= W*0.15;
		Ping.WinWidth		= W*0.07;
		MapName.WinWidth	= W*0.19;
		Players.WinWidth	= W*0.08;
	}
	else
	{	// not showing game type column
		Server.WinWidth		= W*0.36;
		GameType.WinWidth	= 0;
		Ping.WinWidth		= W*0.09;
		MapName.WinWidth	= W*0.21;
		Players.WinWidth	= W*0.1;
	}
}
...
// Obsolete!  UTBrowserServerGrid gets called instead
function JoinServer(UBrowserServerList Server, bool bPlayAsSpectator, bool bAdmin)
{
	...
	// RWS Change, changed from unreal to postal2
		GetPlayerOwner().ClientTravel("postal2://"$Server.IP$":"$Server.GamePort$UBrowserServerListWindow(GetParent(class'UBrowserServerListWindow')).URLAppend$specstring$joinstring, TRAVEL_Absolute, false);
...
function int ByGameType(UBrowserServerList T, UBrowserServerList B)
{
	local int Result;
...
defaultproperties
{
	...
	AutoPingInterval=5
	DefaultHelpText="Shows available game servers.\\nClick to select.  Right-click for options."
}