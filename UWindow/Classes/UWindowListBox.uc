...
// RWS CHANGE: Clear the selected item
...
{
	// RWS CHANGE: Reworked so newly inserted item is selected, and also fixed a problem
	// where you couldn't manually select the new item.
	L.ClearSelectedItem();
	I.Remove();
	Items.AppendItem(I);
	SetSelectedItem(I);
