...
function KeyPressed( int Key, float MouseX, float MouseY )
{
	if(bCanEdit && bKeyDown)
	{
		if( !bControlDown )
		{
			// RWS CHANGE: If ESC was pressed, don't clear out the box - just let the menu go back.
			if(bAllSelected && Key!=0x1B)	// if(bAllSelected)
				Clear();
...
{
	local float W, H, fFullW, fFullH;
	local float TextY;
	local float fAlignOff;
	...
	// RWS CHANGE: 02/02/03 JMI Added adjustment when alignment is centered or right.
	switch (Align)
	{
	case TA_Center:
		TextSize(C, Value, fFullW, fFullH);
		fAlignOff = (WinWidth - fFullW) / 2;
		break;
	case TA_Right:
		TextSize(C, Value, fFullW, fFullH);
		fAlignOff = (WinWidth - fFullW);
		break;
	case TA_Left:
	default:
		fAlignOff = 0;
	...
	ClipText(C, fAlignOff + Offset + 1, TextY,  Value);
	...
	if(bShowCaret)
		ClipText(C, fAlignOff + Offset + W - 1, TextY, "|");
}
