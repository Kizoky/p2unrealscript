...
function LMouseDown(float X, float Y)
{
	...
	if(bAllowSizing && X > Min(WinWidth - 5, ParentWindow.WinWidth - WinLeft - 5) && Y < 12)
	{
		bSizing = True;
...
function MouseMove(float X, float Y)
{
	...
	if(bAllowSizing && bSizing && bMouseDown)
	{
		WinWidth = X;
		if(WinWidth < 1) WinWidth = 1;
...
{
	Super.MouseEnter();
	UWindowGrid(ParentWindow.ParentWindow).MouseEnterColumn(Self);
}
