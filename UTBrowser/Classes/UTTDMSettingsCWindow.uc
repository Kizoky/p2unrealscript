// TEAM DEATHMATCH GAME SETTINGS

class UTTDMSettingsCWindow extends UTTeamSettingsCWindow;
...
function SetupMapOptions()
{
	...
	MaxLivesEdit.SetFont(ControlFont);
	MaxLivesEdit.SetNumericOnly(True);
	MaxLivesEdit.SetMaxLength(3);
	MaxLivesEdit.SetValue(string(class<TeamGame>(BotmatchParent.GameClass).Default.MaxLives));
...
function MaxLivesChanged()
{
	...
	class<TeamGame>(BotmatchParent.GameClass).Default.MaxLives = int(MaxLivesEdit.GetValue());
}
